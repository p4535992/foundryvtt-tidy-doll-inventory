...
import DollInventorySheet from "./scripts/sheet/DollInventorySheet.mjs";
import { dollConfig } from "./scripts/config.js";
import { preloadDollInventoryTemplates } from "./scripts/preloadTemplates.js";
import SheetSettingForm from "./scripts/SheetSettingForm.js";
...
Hooks.once("init", async function () {
    ...
    //loading templates
    loadTemplates(["modules/tidy-doll-inventory/templates/sheet-config.hbs"]);
...
Hooks.on("renderItemSheet", async function (sheet, html, options) {
    ...
    if (dollConfig.dropableTypes.includes(sheet.item.type)) {
        ...
        if (game.user.isGM || game.settings.get("tidy-doll-inventory", "playersCanSetSlot")) {
            let htmlElement = await renderTemplate(
                "modules/tidy-doll-inventory/templates/itemLocationSelector.hbs",
                inventoryLocations,
            );
            ...
            for (let check of html.find("input[data-location]")) {
                check.addEventListener("change", async function () {
                    ...
                    await sheet.item.setFlag("tidy-doll-inventory", "inventoryLocations", inventoryLocations);
                });
            }
        }
    }
